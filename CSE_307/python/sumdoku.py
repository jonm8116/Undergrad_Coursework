...
for i in range(0, len(constraints)):
	constraints[i] = [0 for j in range(9)]
...
class SEARCH_STATE:
	...
	def __init__(self):
		self.avail_mask = [0 for i in range(9)]
		for x in range(0, 9):
			self.avail_mask[x] = [0 for i in range(9)]
			
		self.row_avail_counts = [0 for i in range(9)]
		for x in range(0, 9):
			self.row_avail_counts[x] = [0 for i in range(9)]
			
		self.col_avail_counts = [0 for i in range(9)]
		for x in range(0, 9):
			self.col_avail_counts[x] = [0 for i in range(9)]
		
		self.box_avail_counts = [0 for i in range(3)]
		for x in range(0, 3):
			self.box_avail_counts[x] = [0 for i in range(3)]
			for y in range(0, 3):
				self.box_avail_counts[x][y] = [0 for i in range(9)]
		
		self.val_set = [0 for i in range(9)]
		for x in range(0, 9):
			self.val_set[x] = [0 for i in range(9)]
...
def search_init():
	...
	for i in range(0, 9):
		for j in range(0, 9):
			#print("size " + str(len(pss.avail_mask)))
...
def Solve(level):
	...
	while(FindNextTest(pss, sd)==0):
		if(level==80):
			...
			pssnxt = pss 
			#print("error")
			if(ApplyChoice(pssnxt, sd.test_row, sd.test_col,
			sd.solve_val)==0):
				check_soln(pssnxt)
				...
				print("Apply choice failed at level ")
...
def main():
	...
	for curprob in range(1, nprob+1):
		index = int(inputFile.readline()) 
		if(index != curprob):
			print("Error")
			return -7 
		
		search_init()
		ret=scan_constraints()
		if((ret)!=0):
			return ret 
		if(check_constraints(states[0])!=0):
			print("problem index check constraints")
			return -8 
		...
		if(check_soln(states[0])!=0):
			...
			return -18 
		if(Solve(0)!=0 ):
			print("Error problem index")
			return -9 
		print(str(index))
		for i in range(0,9):
			for j in range(0,9):
				print(str(states[0].val_set[i][j]) + " ")
